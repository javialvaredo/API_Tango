...
class ApiTango(ApiConsultaLive): # en desarrollo

    def __init__(self, base_url: str, api_key: str, company_id: str):
        super().__init__(base_url, api_key, company_id)

    def get_data(self, process: str, page_size: str) -> List[Dict]:
...
class VentanaGrafica(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Entrada de Parámetros para Consultas Live -  API TANGO ")
...
def main(proceso):
    base_url = 'http://cv-tango:17000/Api/Get/'
    api_key = '2f34f26e-c228-4dd2-af3b-616da1112a1e'
    company_id = '4'
    proceso = proceso
    page_size = 10
    
    app = ApiTango(base_url, api_key, company_id)
    datos = app.get_data(proceso, page_size)
    print(datos)
